...
*/
 ...
 if (step->GetPreStepPoint()->GetPhysicalVolume()->GetName()=="Target") flagVolume=1;

  if (processName!="Transportation" && flagVolume==1 && flagProcess!=-1)
  {
    /*
    if (flagParticle==2){
      step->GetTrack()->GetTrackID();
      std::cout<<preStep->GetKineticEnergy()/MeV
                <<'\n'<<  step->GetTrack()->GetTrackID()<<'\n';
    }
    */
  ...
  //  std::cout<<z*0.0000001<<"\n";
    //std::cout<<flagProcess;
  ...
  //  analysisManager->FillNtupleDColumn(14,flagVolume);

    analysisManager->AddNtupleRow();

    //analysisManager->CreateH1("0","totalEnergyDeposit",100,0.,10*MeV);
  }
}
